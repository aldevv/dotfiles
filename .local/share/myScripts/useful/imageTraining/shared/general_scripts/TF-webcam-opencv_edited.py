...
def write_to_file(data):
    ...
    if not os.path.isfile('detections.csv'):
        with open('detections.csv', 'a') as csvfile:
            ...
            writer.writeheader()
            writer.writerow({'Clase': data['label'], 'Ubicación': location,
                             'Hora': datetime.datetime.now().strftime('%d-%m-%Y %H:%M:%S'), 'Precision': data['accuracy']})
            return

    with open(r'detections.csv', 'a', newline='') as csvfile:
        fieldnames = ['Clase', 'Ubicación', 'Hora', 'Precision']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writerow({'Clase': data['label'], 'Ubicación': location,
                         'Hora': datetime.datetime.now().strftime('%d-%m-%Y %H:%M:%S'), 'Precision': data['accuracy']})
...
while True:
    ...
    for i in range(len(scores)):
        if ((scores[i] > MIN_CONF_THRESH) and (scores[i] <= 1.0)):  # if found!!!
            ...
            # Aqui es donde manda a la api de ubidots
            write_to_file(
                {"label": object_name, "accuracy": int(scores[i]*100)})
            if object_name:
                print(object_name)
